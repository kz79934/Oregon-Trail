...
function getOccupation(){
	var t = "<form><div class='container'><ul>\
				<li><input type='radio' id='f-option' name='occupation' value='Banker' onclick = 'displayOcc(this.value)'> <label for='f-option'>Banker</label> <div class='check'></div> </li><br>\
				<li><input type='radio' id='s-option' name='occupation' value='Carpenter' onclick = 'displayOcc(this.value)'> <label for='s-option'>Carpenter</label> <div class='check'><div class='inside'></div></div> </li><br>\
				<li><input type='radio' id='t-option' name='occupation' value='Farmer' onclick = 'displayOcc(this.value)' > <label for='t-option'>Farmer</label> <div class='check'><div class='inside'></div></div> </li><br>\
				<li><input type='radio' id='u-option' name='occupation' value='Outlaw' onclick = 'displayOcc(this.value)' > <label for='u-option'>Outlaw</label> <div class='check'><div class='inside'></div></div> </li><br>\
				<li><input type='radio' id='v-option' name='occupation' value='Cowboy' onclick = 'displayOcc(this.value)' > <label for='v-option'>Cowboy</label> <div class='check'><div class='inside'></div></div> </li><br>\
				<li><input type='radio' id='w-option' name='occupation' value='Merchant' onclick = 'displayOcc(this.value)' > <label for='w-option'>Merchant</label> <div class='check'><div class='inside'></div></div> </li><br>\
				<li><input type='radio' id='x-option' name='occupation' value='Batman' onclick = 'displayOcc(this.value)' > <label for='x-option'>Batman</label> <div class='check'><div class='inside'></div></div> </li><br>\
			</ul></div></form>\
			<div id='info' ></div>\
				<br>\
...
function displayOcc(occupation){
	if(occupation == "Banker" ){
			document.getElementById("info").innerHTML = "<label>Banker has the most starting money in the game but you get least amount of points playing him.</label>";
			supplies[MONEY] = 1600.00;
	}
	else if(occupation == "Carpenter"){
			document.getElementById("info").innerHTML = "<label>The Carpenter starts with an average amount of money, but get more points than the banker.</label>";
			supplies[MONEY] = 800.00;
	}
	else if(occupation == "Farmer" ){
			document.getElementById("info").innerHTML = "<label>You get little starting money, but 3 times as many points has the farmer.</label>";
			supplies[MONEY] = 400.00;
	}
	else if(occupation == "Outlaw"){
			document.getElementById("info").innerHTML = "<label>You start with basically no money, but you can rob people. Be sure not to get arrested as there are dire consequences.</label>";
			supplies[MONEY] = 200.00;
	}
	else if(occupation == "Cowboy" ){
			document.getElementById("info").innerHTML = "<label>The cowboy starts with a below average amount of money, but knows how to take care of it's cattle.</label>";
			supplies[MONEY] = 600.00;
	}
	else if(occupation == "Merchant"){
			document.getElementById("info").innerHTML = "<label>The Merchant starts with a below average amount of money, but gets better deals when trading.</label>";
			supplies[MONEY] = 600.00;
	}
	else if(occupation == "Batman"){
			document.getElementById("info").innerHTML = "<label>You're Batman!</label>";
...
function locationInfo(){
	...
	if(tempTraveled > 0)
		t += "<button class='button' onclick=''><span>Go Fishing</span></button></div>";
	else
		t += "<button class='button' onclick=''><span>Talk to people</span></button>\
